...
// Connect to MongoDB
// Ensure MONGODB_URI is loaded correctly from the .env file.
...
app.post("/applicationForm", upload.single("file"), async (req, res) => {
    try {
        ...
        // This line is now conditional based on your Multer setup.
        // If you are using Cloudinary via the 'storage' object passed to Multer,
        // the file URL will be in req.file.path (Cloudinary's path).
        const documentUrl = req.file.path; 

        const student = new StudentInfo({
            ...formFields,
            documentUrl: documentUrl, // Use the correct path or URL here
        });
...
// Corrected download route to serve the file itself
app.get("/download/:fileName", (req, res) => {
    const fileName = req.params.fileName;
    // Note: This path should be the same path where Multer saves the file.
    const filePath = path.join(__dirname, "uploadedFile", fileName);

    try {
        if (fs.existsSync(filePath)) {
            // Use res.sendFile() to send the file for download.
            // The root option specifies the base directory for the file path.
